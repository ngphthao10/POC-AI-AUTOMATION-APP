...
copy_playwright_browsers() {
    print_step "Copying Playwright browsers to dist folder..."
    ...
    if [ -n "$PLAYWRIGHT_CACHE" ] && [ -d "$PLAYWRIGHT_CACHE" ]; then
        print_step "Found Playwright browsers at: $PLAYWRIGHT_CACHE"
        mkdir -p dist/.playwright-browsers
        cp -r "$PLAYWRIGHT_CACHE"/* dist/.playwright-browsers/
        print_success "Playwright browsers copied (~300MB)"
    else
        print_warning "Playwright browsers not found in cache"
        print_warning "End users will need to install browsers separately"
    fi
}
...
if [ -d "$SCRIPT_DIR/.playwright-browsers" ]; then
    export PLAYWRIGHT_BROWSERS_PATH="$SCRIPT_DIR/.playwright-browsers"
    echo "✓ Using bundled Playwright browsers"
else
    echo "⚠️  Bundled browsers not found, using system installation"
fi
...
Issue: macOS "Cannot verify that it is free from malware"
Fix:   Run this command in the dist folder:
       xattr -cr .
       Then run ./launch.sh normally
       (This removes the quarantine flag from all files)
...
💡 RECOMMENDED: Use Windows for distribution
   Windows doesn't have code signing issues like macOS
   Simply run build.bat on Windows for hassle-free distribution
